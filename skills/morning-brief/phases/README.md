...
|---|---|---|---|
| [scribbles](scribbles.md) | Sweep Apple Note "Scribbles" → tasks/events/notes via `/scribbles` | `scribbles_inbox: true` | — |
...
1. **scribbles** — sweep inbox so new tasks land before briefing reads them
2. **briefing** — quick orientation for the day
3. **research** — pull external research source (if configured)
4. **feeds** — pull user-defined newsletters/RSS into context
5. **news** — scan, sync, propose, draft (uses research + feeds context if those ran)

The order matters: scribbles must run before briefing so newly captured tasks show up in today's task list; news depends on research and feeds being in context first.

## Single-phase invocation

- `/morning-brief scribbles` — just sweep the Apple Note inbox.
