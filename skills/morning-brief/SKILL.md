...
|---|---|---|
| `scribbles` | Sweeps the Apple Note "Scribbles" for new items and files them as tasks/events/notes via `/scribbles` — runs *before* `briefing` so new tasks show up in today's task health view | Config flag `scribbles_inbox: true` |
...
- **No descriptor** → run every enabled phase in order: `scribbles` → `briefing` → `research` → `feeds` → `news`. Skip phases whose config flag is false.
- **Single phase name** (`scribbles`, `briefing`, `research`, `feeds`, `news`) → run just that phase, passing the rest of the descriptor as its argument (e.g., `/morning-brief research 2026-04-15` → research phase with date `2026-04-15`).
- **Free-form descriptor** → infer the closest phase. "give me the brief" → fan out. "news of the day" → `news`. "fetch newsletters" → `feeds`. "pull today's research" → `research`. "scribbles" / "inbox" / "sweep scribbles" → `scribbles`.
...
When fanning out, present sections in order: **Scribbles** (only if N > 0), **Briefing**, **Research**, **Feeds**, **News**. Skipped phases get a one-line note ("research skipped — `daily_research: false`" or "research skipped — weekend"). `scribbles` runs quietly when N = 0 (no section rendered).
...
- Config flags live in `.claude/constitution/config.yaml`: `news_scan`, `daily_research`, `feeds`, `scribbles_inbox`, plus `self_reflection` (read by `briefing` for the reflection nudge) and the lighter ritual flags (`gratitude`, `daily_curiosity`, etc.) which `briefing` reads.
